                    ...
                    if len(obs) > 0:
                        for obs_list_current in obs:
                            ...
                            if obs_list_current[s_custom] is not None or obs_list_current[s_new] is not None or obs_list_current[s_obs] is not None:
                                ...
                                if collected != 0 or observed != 0 or survey_list_current[survey_list_header.index('section_condition')] == 'yes':

                                    #Build object for output
                                    output_list.append(cls.resultObject(survey_list_current, 
                                                        loc_list_current, 
                                                        shot_list_current, 
                                                        obs_list_current, 
                                                        sample_list_current,
...
out_list_length = len(output_list)
    ...
    if s_custom is not None or s_samp is not None: 
        ...
        if skip_samp == FALSE:
            ...
            for i in sampling_list:
                if PGID == output_list[i].locations[loc_list_header.index('ParentGlobalID')] and species == output_list[i].observations[obs_list_header.index('species_obs')]:

                    #Fix collected number:
                    section_num = (0 if output_list[i].shots[shot_list_header.index('section_number')] is None else output_list[i].shots[shot_list_header.index('section_number')])
                    func.adjust_species_count(sample_list_current, output_list, PGID, section_num, species,survey_list_header, obs_list_header, sample_list_header, loc_list_header, shot_list_header)                                    
                    ...
                    output_list.append(cls.resultObject(output_list[i].surveys, 
                                                output_list[i].locations, 
                                                output_list[i].shots, 
                                                obs_list_current, 
                                                sample_list_current,
                                                creator))  
...
output_list_header = survey_list_header_edit + loc_list_header_edit + shot_list_header_edit + obs_list_header_edit + sample_list_header_edit
#Append GlobalID List and creator:
output_list_header += ['Survey_GlobalID', 'Site_GlobalID', 'Shot_GlobalID', 'Obs_GlobalID', 'Sample_GlobalID', 'Creator']

for i in output_list:
    i.collate(output_list_header)

output_list_header.pop(output_list_header.index('species_samp'))
output_list_header[output_list_header.index('species_obs')] = 'species'
...
row_count = 1
func.write_row(ws_write, row_count, 1, output_list_header)
